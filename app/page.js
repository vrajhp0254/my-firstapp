...
function page() {
  ...
  if (mainTask.length>0) {
    renderTask=mainTask.map((t,i)=>{
      ...
      <button onClick={()=>{deletehandler(i)}}
       className=" bg-red-600 text-white rounded px-1 py-2">Delete</button>
      </li>
      )
    })
  }
  return (
    <>
      <div className="bg-black text-white h-[80px] text-5xl text-center uppercase pt-5">
        my todo-list
      </div>
      <form
      ...
      >
        <input
          className="border-2 border-black  w-[400px] p-1"
          type="text"
        ...
        />
        <button className="border-2 border-black w-[100px] h-[27.2px] rounded bg-black text-white ">
          Add task
        </button>
      </form>
